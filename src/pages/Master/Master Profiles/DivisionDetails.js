...
//   const formConfig = formConfigs.divisionDetailsForm;
...
//       });
//       const result = await response.json();
//       setRows(result.filter((row) => row.nDivProDtlID));
//       setIsLoading(false);
//     } catch (error) {
...
//     setIsLoading(true);
//     e.preventDefault();
//     const method = formData.nDivProDtlID ? "PUT" : "POST";
...
//         if (!formData.nDivProDtlID) {
//           setRows((prevRows) => [...prevRows, result]);
//         } else {
//           setRows((prevRows) =>
//             prevRows.map((row) =>
//               row.nDivProDtlID === result.nDivProDtlID ? result : row
//             )
//           );
//         }
...
//       // Filter out any invalid rows (rows that don't have a nDivProDtlID)
//       const validRows = result.filter((row) => row.nDivProDtlID);
...
//           </StyledButton>
//           <StyledButton
//             onClick={() => handleDelete(params.row.nDivProDtlID)}
//             bgColor={Colortheme.buttonBg}
//             bgColorHover={"red"}
...
//   const handleDelete = async (nDivProDtlID) => {
//     setIsLoading(true);
//     try {
...
//           Authorization: `Bearer ${token}`,
//         },
//         body: JSON.stringify({ nDivProDtlID }),
//       });
//       const result = await response.json();
//       if (response.ok) {
//         console.log(result.message);
//         setRows((prevRows) =>
//           prevRows.filter((row) => row.nDivProDtlID !== nDivProDtlID)
//         );
//         setSearchKeyword("");
//         setIsLoading(false);
...
//     .filter((row) => row.nDivProDtlID) // Ensure each row has a nDivProDtlID
//     .map((row) => {
...
//   return (
//     <MainContainerCompilation title="Division Details">
//       <Box>
//         {isLoading ? (
//           <MutatingDots
//             visible={true}
...
//               {formData.nDivProDtlID && isFormVisible && (
//                 <StyledButton
...
//                         bgColorHover={Colortheme.buttonBgHover}
//                       >
//                         {formData.nDivProDtlID ? "Save" : "Create"}
//                       </StyledButton>
//                       <StyledButton
//                         type="button"
...
export default FinancialSubProfile;
