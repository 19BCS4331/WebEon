...
const FinancialSubProfile = () => {
  ...
  const formConfig = formConfigs.financialSubCodesForm;
  ...
  const fetchOptions = async (field, value) => {
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ vFinType: value }),
      });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const result = await response.json();
      setRows(result.filter((row) => row.nSFID));
      setIsLoading(false);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    setIsLoading(true);
    e.preventDefault();
    const method = formData.nSFID ? "PUT" : "POST";
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!formData.nSFID) {
          ...
          setRows((prevRows) =>
            prevRows.map((row) => (row.nSFID === result.nSFID ? result : row))
          );
        }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Filter out any invalid rows (rows that don't have a nSFID)
      const validRows = result.filter((row) => row.nSFID);
    ...
    // Add other columns as needed
    {
      ...
      renderCell: (params) => (
        <Box display={"flex"} gap={2}>
          ...
          <StyledButton
            onClick={() => handleDelete(params.row.nSFID)}
            bgColor={Colortheme.buttonBg}
            bgColorHover={"red"}
  ...
  const handleDelete = async (nSFID) => {
    setIsLoading(true);
    try {
      const response = await fetch(`${formConfig.endpoint}/delete`, {
        ...
        body: JSON.stringify({ nSFID }),
      });
      const result = await response.json();
      if (response.ok) {
        console.log(result.message);
        setRows((prevRows) => prevRows.filter((row) => row.nSFID !== nSFID));
        setSearchKeyword("");
        setIsLoading(false);
  ...
  const filteredRows = rows
    .filter((row) => row.nSFID) // Ensure each row has a nSFID
    .map((row) => {
  ...
  return (
    <MainContainerCompilation title="Financial Sub Codes">
      <Box>
        {isLoading ? (
          <MutatingDots
            visible={true}
